...
- A follow-up build attempt for `CsScript` began successfully but timed out while rebuilding prerequisite targets such as `CsCore`, so the next unresolved frontier is compiler-driven Script diagnosis rather than path drift.
...
4. Continue Stage A `CsScript` recovery by moving from path-drift reduction into actual compiler-driven diagnosis: get the build to the first direct `CsScript` compile errors and classify them.
