...
12. Verify popup-aware mouse enter/leave receiver updates do not blindly promote unrelated-owner widgets while a popup is active.
13. Verify popup-aware synthetic enter/leave generation does not blindly target unrelated-owner widgets while a popup is active.
...
The overlay smoke path now exercises the richer HUD scaffold, including popup-stack summaries, focus/target popup-relationship summaries, owner-group summaries, blocker-group summaries, preset-oriented panel modes, blocked-only filtering, filtered popup-panel visibility, and panel-based blocked-route visibility, rather than only a plain text rendering shell.
