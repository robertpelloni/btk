...
- A follow-up contraction pass over `src/script/CMakeLists.txt` then reduced the restored Script JavaScriptCore path drift to zero missing references and allowed `WITH_SCRIPT=ON` to reach a successful experimental configure pass, shifting the next frontier to compiler-driven `CsScript` diagnostics.
