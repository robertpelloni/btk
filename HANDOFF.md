...
4. Continue Stage A `CsScript` recovery with a private-API adaptation inventory for the first failing hotspots: `qscriptengine_p.h`, `qscriptactivationobject*`, `qscriptstaticscopeobject*`, `qscriptobject*`, `qscriptqobject*`, and `qscriptdeclarativeclass*`.
