...
- A generated JavaScriptCore drift audit now shows the restored Script baseline is mostly aligned with the current embedded tree (408 existing references vs 56 missing), which narrows the next work to clustered adaptation around `runtime`, `pcre`, `wrec`, and `yarr` rather than broad subsystem uncertainty.
